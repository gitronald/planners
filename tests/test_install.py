...
# --- mode: switching, PATH, and repo bootstrap -------------------------------
...
def test_remove_stale_holder_noop_when_root_is_home(
    ...
    # Running `install --global` from $HOME makes the local and global holder
...
def test_ensure_plans_dir_is_idempotent(tmp_path: Path) -> None:
    assert ensure_plans_dir(tmp_path) is True
    assert (tmp_path / ".planners" / "plans").is_dir()
    assert ensure_plans_dir(tmp_path) is False
...
def test_check_rule_drifted_for_stale_local_after_global_switch(
    ...
    # per-repo rule (old version stamp) left behind by a --global switch. Both are
