...
* **local** (default) — the CLI is a per-repo dev dependency invoked as
  ``uv run planners``; the holder lives at ``<repo>/.claude/skills/planners/``.
* **global** (``--global``) — the CLI is on ``PATH`` (``uv tool install``) and
...
recovers it without re-passing ``--global``. What is always per-repo regardless
...
def _stamp(version: str, mode: Mode) -> str:
    ...
    (recovered by ``check`` without re-passing ``--global``).
    """
    return (
...
def check_rule(root: Path, version: str) -> RuleCheck:
    ...
    * **Stale leftover** — a per-repo rule that *still matches* its render but
      ...
      one, e.g. left behind by ``install --global --no-rule`` (which skips the
...
def ensure_plans_dir(root: Path) -> bool:
    """Create ``.planners/plans/`` if absent; returns ``True`` if it was created.

    Global mode bootstraps the per-repo plans folder so a non-uv repo (no
    ``pyproject.toml``, no dep) is ready to hold plans right after install.
    """
